<template>
    <div class="published-view p-6">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">{{ t("published.title") }}</h1>
            <p class="text-gray-500 mt-1">{{ t("published.subtitle") }}</p>
        </div>

        <!-- Loading state -->
        <div v-if="loading" class="flex justify-center py-12">
            <div class="text-gray-500">{{ t("published.loadingPublished") }}</div>
        </div>
        ...
        <div v-else class="bg-white rounded-lg shadow overflow-hidden">
            ...
            <div v-if="!loading && richPods.length === 0" class="text-center py-12">
                <Icon icon="ion:globe-outline" class="w-12 h-12 mx-auto text-gray-400 mb-3" />
                <p class="text-gray-500">{{ t("published.noPublished") }}</p>
                <p class="text-sm text-gray-400 mt-1">{{ t("published.publishFirst") }}</p>
            </div>
        </div>
    </div>
</template>
...
async function loadMore() {
    ...
    try {
        ...
        console.error("Error loading more published RichPods:", err);
        await loadRichPods();
    } finally {
...
async function loadRichPods() {
    ...
    try {
        ...
        console.error("Error loading published RichPods:", err);
        error.value = err instanceof Error ? err.message : t("published.loadFailed");
    } finally {
        loading.value = false;
    }
}
...
async function handleDelete(id: string) {
    if (!confirm(t("published.deleteConfirm"))) {
        return;
    }
    ...
    try {
        ...
        alert(err instanceof Error ? err.message : t("published.deleteFailed"));
    }
}
