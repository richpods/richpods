    ...
    >
        <div class="bg-white rounded-lg w-full max-w-2xl max-h-[85vh] flex flex-col">
            <div class="flex items-center justify-between p-4 border-b">
                <h3 class="text-lg font-medium">{{ t("markdownEditor.wikidataSearch.title") }}</h3>
                <button
                    type="button"
                    class="text-gray-500 hover:text-gray-700 p-1"
                    @click="handleClose"
                    :aria-label="t('markdownEditor.wikidataSearch.closeModalAriaLabel')"
                >
                    ✕
                </button>
            </div>
            ...
            <div class="p-4 border-b space-y-3">
                <div class="relative">
                    ...
                    <input
                        ...
                        v-model="searchQuery"
                        type="text"
                        :placeholder="t('markdownEditor.wikidataSearch.searchPlaceholder')"
                ...
                <div class="flex flex-wrap gap-2">
                    <span class="text-sm text-gray-600 self-center">
                        {{ t("markdownEditor.wikidataSearch.searchIn") }}
                    </span>
                    <button
            ...
            <div class="flex-1 overflow-y-auto p-4">
                <div v-if="isLoading" class="flex items-center justify-center py-8">
                    <RipoSpinner
                        :size="24"
                        color="#2563eb"
                        :label="t('markdownEditor.wikidataSearch.searching')"
                    />
                    <span class="ml-2 text-gray-600">{{
                        t("markdownEditor.wikidataSearch.searching")
                    }}</span>
                </div>
                ...
                >
                    {{
                        t("markdownEditor.wikidataSearch.noResultsFoundFor", { query: searchQuery })
                    }}
                </div>

                <div v-else-if="groupedResults.length === 0" class="py-8 text-center text-gray-500">
                    {{ t("markdownEditor.wikidataSearch.enterSearchTerm") }}
                </div>
                        ...
                        <div class="space-y-1">
                            ...
                            >
                                <div class="flex items-start justify-between gap-2">
                                    ...
                                    >
                                        {{ t("markdownEditor.wikidataSearch.selected") }}
                                    </span>
                                </div>
                            </button>
                        </div>
            ...
            <div class="flex items-center justify-between gap-3 p-4 border-t bg-gray-50">
                <div class="text-sm text-gray-500">
                    <span v-if="selectedResult">
                        {{ t("markdownEditor.wikidataSearch.generateIn") }}
                    ...
                    >
                        <span v-if="isGenerating" class="flex items-center gap-2">
                            <RipoSpinner
                                ...
                                :label="t('markdownEditor.wikidataSearch.generating')"
                            />
                            {{ t("markdownEditor.wikidataSearch.generating") }}
                        </span>
                        <span v-else>{{
                            t("markdownEditor.wikidataSearch.useSelectedEntity")
                        }}</span>
                    </button>
                </div>
            </div>
...
const emit = defineEmits<{
    (e: "close"): void;
    (e: "select", template: WikidataTemplate): void;
}>();
...
async function performSearch() {
    ...
    try {
        ...
        error.value =
            e instanceof Error ? e.message : t("markdownEditor.wikidataSearch.searchFailed");
        groupedResults.value = [];
    } finally {
...
async function handleSelect() {
    ...
    try {
        const template = await generateWikidataTemplate(
        ...
        error.value =
            e instanceof Error ? e.message : t("markdownEditor.wikidataSearch.generateFailed");
    } finally {
